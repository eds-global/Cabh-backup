...
function logMsg($logMessage){
    // Your log file path
    $logFilePath = "/home/uxjb3x180e71/public_html/cabh_iaq_dashboard/cron/15minlogfile.txt";
...
function getData($deviceIDs, $startdt, $enddt){    
    ...
    $select_query = " insert into reading_15min  Select b.deviceID, '$enddt' as datetime1, max(pm25) as max_pm25, max(pm10) as max_pm10, max(aqi) as max_aqi, max(co2) as max_co2, max(voc) as max_voc, max(temp) as max_temp, max(humidity) as max_hum, min(pm25) as min_pm25, min(pm10) as min_pm10, min(aqi) as min_aqi, min(co2) as min_co2, min(voc) as min_voc, min(temp) as min_temp, min(humidity) as min_hum,
...
function getCPCBData($deviceIDs, $startdt, $enddt){    
    ...
    $devices = "'". implode("', '", $deviceIDs). "'";
    print $devices;
    $select_query = " insert into cpcb_15min  Select b.deviceID, '$enddt' as datetime1, max(pm25) as max_pm25, max(pm10) as max_pm10, max(aqi) as max_aqi, max(temp) as max_temp, max(humidity) as max_hum, min(pm25) as min_pm25, min(pm10) as min_pm10, min(aqi) as min_aqi, min(temp) as min_temp, min(humidity) as min_hum,
    ...
    //$result = $conn->query($select_query);
    if ($conn->query($select_query) === TRUE) {
        logMsg("$enddt : CPCB Data inserted")  ;
        $conn->close();
        return "success";
    }
    else{
