...
function updateLastData($data){
    ...
    foreach($data as $dt){
        ...
        $lastdata = $dt['datetime'];

        $givenDateTime = new DateTime($lastdata);
        $current_dt = new DateTime();
        //echo $current_dt->format('Y-m-d H:i:s') . "\n";
        $timeDifference = $current_dt->diff($givenDateTime);
        $minutesDifference = $timeDifference->days * 24 * 60 + $timeDifference->h * 60 + $timeDifference->i;
        ...
        $conn = getConnection();
        $select_query = " Select DISTINCT deviceID from last_data ";
        $result = $conn->query($select_query);
        $devices = array();
        if ($result) {
            while($row = $result->fetch_assoc()){
        ...
        if(in_array($deviceid, $devices)) { 
            ...
            $update_query = "Update last_data set datetime = '" . $lastdata->format('Y-m-d H:i:s') . "' where deviceID = '" . $deviceid . "' ";
            ...
            echo "not found"; 
            $insert_query = "Update last_data set datetime = '" . $lastdata->format('Y-m-d H:i:s') . "' where deviceID = '" . $deviceid . "' ";
